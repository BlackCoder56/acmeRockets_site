  ...
  <header class="bg-teal-700 text-white sticky top-0 z-10">
    <section class="max-w-4xl mx-auto p-4 flex justify-between items-center">
      ...
      <div>
       ...
       <button
  @click="toggleMenu"
  :class="{ 'toggle-btn': isMenuOpen }"
  class="text-3xl md:hidden cursor-pointer relative w-8 h-8 isolate"
>
  <div
    class="bg-white w-8 h-1 rounded absolute top-5 -mt-0.5 transition-all duration-500
           before:content-[''] before:absolute before:w-8 before:h-1 before:rounded before:bg-white before:-translate-y-2.5 before:transition-all before:duration-500
           after:content-[''] after:absolute after:w-8 after:h-1 after:rounded after:bg-white after:translate-y-2.5 after:transition-all after:duration-500"
  ></div>
</button>
